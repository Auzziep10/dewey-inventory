...
import { Plus, Package, Box, ChevronRight, QrCode, Printer, X, BarChart3, Layers, Tag, Copy, Search, Map, Upload, Loader2 } from 'lucide-react';
...
export function EventsTab({ onJumpToWarehouse, initialActivePalletId }: { onJumpToWarehouse?: (palletId: string, zone: string, warehouseId?: string) => void, initialActivePalletId?: string | null }) {
  ...
  const [newPalletForm, setNewPalletForm] = useState({ name: '', type: 'Road Case', width: 4, depth: 2.5, height: 3 });
  ...
  const [newItemForm, setNewItemForm] = useState({ sku: '', name: '', quantity: 1, photoUrl: '' });
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
          ...
          const storageRef = ref(storage, `event_items/${Date.now()}_${Math.random().toString(36).substring(7)}.${fileExtension}`);
          await uploadBytes(storageRef, file);
          const url = await getDownloadURL(storageRef);
          
          setNewItemForm(prev => ({...prev, photoUrl: url}));
      } catch (error) {
  ...
  const [editItemForm, setEditItemForm] = useState({ name: '', sku: '', quantity: 1, photoUrl: '' });
  ...
  const handleEditPhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
          ...
          setEditItemForm(prev => ({...prev, photoUrl: url}));
      } catch (error) {
          console.error("Error uploading photo:", error);
          alert("Failed to upload photo. Please try again.");
      } finally {
  ...
  useEffect(() => {
     const q = query(collection(db, 'event_items'));
  ...
  const handleCreatePallet = async () => {
      if (!newPalletForm.name.trim()) return;
      const newPallet: Pallet = {
          id: `evt_${Date.now()}`,
          name: newPalletForm.name,
          type: newPalletForm.type,
          dimensions: { width: newPalletForm.width, depth: newPalletForm.depth, height: newPalletForm.height },
          createdAt: Date.now(),
          boxes: []
      };
      await setDoc(doc(db, 'event_items', newPallet.id), newPallet);
      setNewPalletForm({ name: '', type: 'Road Case', width: 1.2, depth: 0.6, height: 0.8 });
  ...
  const handleCreateBox = async () => {
      ...
      await setDoc(doc(db, 'event_items', activePallet.id), updatedPallet);
      setNewBoxName('');
      setIsAddingBox(false);
      setActiveBoxId(newBox.id);
  };
  
  const handleAddLooseItemClick = async () => {
      if (!activePallet) return;
      let defaultBox = activePallet.boxes.find(b => b.name === 'Loose Items');
      if (!defaultBox) {
          const newBox: BoxType = {
              id: `box_${Date.now()}`,
              name: 'Loose Items',
          ...
          const updatedPallet = {
              ...activePallet,
              boxes: [...activePallet.boxes, newBox]
          };
          await setDoc(doc(db, 'event_items', activePallet.id), updatedPallet);
          setActiveBoxId(newBox.id);
          setIsAddingItem({boxId: newBox.id});
      } else {
          setActiveBoxId(defaultBox.id);
          setIsAddingItem({boxId: defaultBox.id});
      }
  };
  ...
  const handleCreateItem = async (boxId: string) => {
      ...
      const newItem: Item = {
          ...
          size: '', // Force empty for event items
      ...
      setNewItemForm({ sku: '', name: '', quantity: 1, photoUrl: '' });
  };
  ...
  const handleDeleteBox = async (palletId: string, boxId: string, boxName: string) => {
      ...
      try {
          ...
          await setDoc(doc(db, 'event_items', palletId), updatedPallet);
  ...
  const handleUpdateBoxName = async (palletId: string, boxId: string, newName: string) => {
      ...
      await setDoc(doc(db, 'event_items', palletId), { ...p, boxes: updatedBoxes });
  ...
  const handleUpdatePalletName = async (palletId: string, newName: string) => {
      ...
      const p = pallets.find(p => p.id === palletId);
      if(!p) return;
      await setDoc(doc(db, 'event_items', palletId), { ...p, name: newName });
  ...
  const handleUpdateItemDetails = async (palletId: string, boxId: string, itemId: string) => {
      ...
      const updatedBoxes = p.boxes.map(b => {
          if (b.id === boxId) {
              return { ...b, items: b.items.map(i => i.id === itemId ? { ...i, name: editItemForm.name, sku: editItemForm.sku, quantity: editItemForm.quantity, photoUrl: editItemForm.photoUrl } : i) };
          }
          return b;
      });
      await setDoc(doc(db, 'event_items', palletId), { ...p, boxes: updatedBoxes });
  ...
  const handleMoveBox = async (targetPalletId: string, palletId: string, boxId: string) => {
      ...
      await setDoc(doc(db, 'event_items', sourcePallet.id), { ...sourcePallet, boxes: updatedSourceBoxes });
      await setDoc(doc(db, 'event_items', targetPallet.id), { ...targetPallet, boxes: updatedTargetBoxes });
       ...
       <div className="w-80 h-full flex flex-col bg-white rounded-card border border-brand-border shadow-sm shrink-0 overflow-hidden">
          <div className="p-4 border-b border-brand-border/60 bg-brand-bg/50">
             ...
             {!isAddingPallet ? (
                 ...
                 >
                     <Plus size={14} /> Create Event Payload
                 </button>
             ) : (
                 <div className="bg-white p-3 rounded-xl border border-brand-border shadow-sm animate-in zoom-in-95">
                     <select value={newPalletForm.type} onChange={(e) => setNewPalletForm({...newPalletForm, type: e.target.value})} className="w-full mb-2 p-2 text-[10px] font-bold uppercase tracking-widest bg-brand-bg border border-brand-border rounded-lg outline-none focus:border-brand-primary cursor-pointer">
                         <option value="Road Case">Road Case</option>
                         <option value="Pallet">Industrial Pallet</option>
                     ...
                     <input 
                         type="text" 
                         autoFocus
                         placeholder="Item Name (e.g. DJ Booth)" 
                         value={newPalletForm.name}
                         onChange={e => setNewPalletForm({...newPalletForm, name: e.target.value})}
                     ...
                     <div className="flex gap-2 mb-3">
                         <div>
                             <label className="text-[8px] uppercase font-bold tracking-widest text-brand-secondary">Width ({newPalletForm.type === 'Box' ? 'in' : 'ft'})</label>
                             <input type="number" step="0.1" value={newPalletForm.width} onChange={e => setNewPalletForm({...newPalletForm, width: parseFloat(e.target.value) || 0.1})} className="w-full text-xs font-semibold p-1.5 bg-brand-bg border border-brand-border rounded outline-none focus:border-brand-primary text-center" />
                         </div>
                         <div>
                             <label className="text-[8px] uppercase font-bold tracking-widest text-brand-secondary">Depth ({newPalletForm.type === 'Box' ? 'in' : 'ft'})</label>
                             <input type="number" step="0.1" value={newPalletForm.depth} onChange={e => setNewPalletForm({...newPalletForm, depth: parseFloat(e.target.value) || 0.1})} className="w-full text-xs font-semibold p-1.5 bg-brand-bg border border-brand-border rounded outline-none focus:border-brand-primary text-center" />
                         </div>
                         <div>
                             <label className="text-[8px] uppercase font-bold tracking-widest text-brand-secondary">Height ({newPalletForm.type === 'Box' ? 'in' : 'ft'})</label>
                             <input type="number" step="0.1" value={newPalletForm.height} onChange={e => setNewPalletForm({...newPalletForm, height: parseFloat(e.target.value) || 0.1})} className="w-full text-xs font-semibold p-1.5 bg-brand-bg border border-brand-border rounded outline-none focus:border-brand-primary text-center" />
                         </div>
                     </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
             {[...filteredPallets].sort((a, b) => {
                 ...
                 >
                     ...
                     <div className={`text-[10px] font-bold uppercase tracking-widest mt-2 flex items-center gap-2 opacity-80 ${activePalletId === p.id ? 'text-neutral-300' : 'text-brand-secondary'}`}>
                         <span className="flex items-center gap-1"><Layers size={12} /> {p.boxes.length} Boxes</span>
                         <span>•</span>
                         <span>{p.type || 'Pallet'}</span>
                     </div>
                 </div>
             ))}
                             ...
                             <>
                                 <div className="flex items-center gap-2 mb-1.5">
                                    <span className="bg-brand-bg text-brand-secondary border border-brand-border px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-widest">
                                        {activePallet.type || 'Pallet'}
                                    </span>
                                    {activePallet.dimensions && (
                                       <span className="text-[9px] uppercase font-bold tracking-widest text-brand-secondary">
                                           {activePallet.dimensions.width}{activePallet.type === 'Box' ? 'in' : 'ft'} W × {activePallet.dimensions.depth}{activePallet.type === 'Box' ? 'in' : 'ft'} D × {activePallet.dimensions.height}{activePallet.type === 'Box' ? 'in' : 'ft'} H
                                       </span>
                                    )}
                                 </div>
                                 <h3 
                             ...
                             <div className="flex flex-col gap-2">
                                 <div className="flex gap-2 mb-2">
                                     <button 
                                         ...
                                         className="flex-1 bg-white border border-brand-border text-brand-primary py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest hover:border-brand-primary transition-colors shadow-sm flex items-center justify-center gap-2"
                                     >
                                         <Plus size={14} /> Add Box
                                     </button>
                                     <button 
                                         onClick={handleAddLooseItemClick}
                                         className="flex-1 bg-brand-primary text-white py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-colors shadow-sm flex items-center justify-center gap-2"
                                     >
                                 ...
                                 {activePallet.boxes?.filter(b => b.name !== 'Loose Items').length > 0 && (
                                     <button 
                         ...
                         {(() => {
                             ...
                             >
                                 {box.name !== 'Loose Items' && (
                                  ...
                                  <div className={`${box.name === 'Loose Items' ? 'pl-2' : 'pl-6'} flex flex-col gap-2 ${box.name === 'Loose Items' ? '' : 'mt-1'}`}>
                                      {box.name !== 'Loose Items' && (
                                      ...
                                      {box.items.length > 0 ? (
                                          <div className={`flex flex-col gap-2 ${box.name === 'Loose Items' ? '' : 'border-t border-brand-border/50 pt-2'}`}>
                                                      ...
                                                      <div className="flex flex-wrap gap-1.5">
                                                          {items.map((i, idx) => (
                                                              <span key={idx} className="bg-brand-bg border border-brand-border text-brand-secondary px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-widest">
                                                                  Qty: <span className="text-brand-primary">{i.quantity}</span>
                                                              </span>
                                                          ))}
                                                      </div>
                                                  </div>
                                          ...
                                          <div className="text-[10px] font-bold text-brand-secondary italic">Adding Item...</div>
                                      ) : null}
                                  </div>
                             </div>
                         );
                                    ...
                                    <div className="bg-white border-2 border-brand-primary p-5 rounded-2xl shadow-lg mb-6 animate-in slide-in-from-top-4">
                                        ...
                                        <div className="grid grid-cols-12 gap-4">
                                            <div className="col-span-3">
                                                ...
                                                <input list="inventory-skus" name="item_sku" autoComplete="on" type="text" value={newItemForm.sku} onChange={e => setNewItemForm({...newItemForm, sku: e.target.value})} className="w-full text-xs font-semibold p-2.5 bg-brand-bg border border-brand-border rounded-lg outline-none focus:border-brand-primary" placeholder="e.g. CBL-PWR" />
                                            </div>
                                            <div className="col-span-6">
                                                <label className="block text-[9px] font-bold uppercase tracking-widest text-brand-secondary mb-1">Item Name</label>
                                                <input list="inventory-names" name="item_garment_name" autoComplete="on" type="text" value={newItemForm.name} onChange={e => setNewItemForm({...newItemForm, name: e.target.value})} className="w-full text-xs font-semibold p-2.5 bg-brand-bg border border-brand-border rounded-lg outline-none focus:border-brand-primary" placeholder="25ft Power Cable" />
                                            </div>
                                            <div className="col-span-3">
                                                <label className="block text-[9px] font-bold uppercase tracking-widest text-brand-secondary mb-1">Units (Qty)</label>
                                            ...
                                            <div className="col-span-12">
                                                <label className="block text-[9px] font-bold uppercase tracking-widest text-brand-secondary mb-1">Photo (Optional)</label>
                                                <div className="flex gap-2">
                                                    {newItemForm.photoUrl ? (
                                                        <div className="relative w-12 h-12 rounded-lg overflow-hidden border border-brand-border group shrink-0">
                                                            <img src={newItemForm.photoUrl} alt="Preview" className="w-full h-full object-cover" />
                                                            <button onClick={() => setNewItemForm({...newItemForm, photoUrl: ''})} className="absolute inset-0 bg-black/50 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                                                <X size={14} />
                                                            </button>
                                                        </div>
                                                    ) : (
                                                        <label className={`bg-brand-bg border border-brand-border rounded-lg h-12 px-4 flex items-center justify-center shrink-0 cursor-pointer hover:bg-brand-primary/10 hover:border-brand-primary/30 transition-all ${isUploadingPhoto ? 'opacity-50 pointer-events-none' : ''}`}>
                                                            {isUploadingPhoto ? <Loader2 size={16} className="text-brand-secondary animate-spin" /> : <Upload size={16} className="text-brand-secondary" />}
                                                            <span className="text-[10px] uppercase font-bold text-brand-secondary ml-2">{isUploadingPhoto ? 'Uploading...' : 'Upload Image'}</span>
                                                            <input type="file" accept="image/*" className="hidden" onChange={handlePhotoUpload} />
                                                        </label>
                                                    )}
                                                    <input type="text" value={newItemForm.photoUrl} onChange={e => setNewItemForm({...newItemForm, photoUrl: e.target.value})} className="flex-1 text-xs font-semibold px-3 h-12 bg-brand-bg border border-brand-border rounded-lg outline-none focus:border-brand-primary" placeholder="Or paste image URL here..." />
                                                </div>
                                            </div>
                                        </div>
                                                ...
                                                {editingItemId === item.id ? (
                                                    <div className="flex flex-col gap-2 p-3 bg-brand-bg rounded-lg border border-brand-border">
                                                        <div className="flex gap-2">
                                                            ...
                                                            <input type="number" min="1" value={editItemForm.quantity} onChange={e => setEditItemForm({...editItemForm, quantity: parseInt(e.target.value)||1})} className="w-16 text-xs font-semibold p-2 bg-white border border-brand-border rounded outline-none focus:border-brand-primary text-center" title="Quantity" />
                                                        </div>
                                                        <input list="inventory-skus" name="item_sku" autoComplete="on" type="text" value={editItemForm.sku} onChange={e => setEditItemForm({...editItemForm, sku: e.target.value})} className="w-full text-[10px] p-2 bg-white border border-brand-border rounded outline-none uppercase focus:border-brand-primary" placeholder="SKU (Opt)" onKeyDown={e => e.key === 'Enter' && handleUpdateItemDetails(activePallet.id, activeBox.id, item.id)} />
                                                        
                                                        <div className="flex gap-2">
                                                            {editItemForm.photoUrl ? (
                                                                <div className="relative w-8 h-8 rounded border border-brand-border group shrink-0 overflow-hidden">
                                                                    <img src={editItemForm.photoUrl} alt="Preview" className="w-full h-full object-cover" />
                                                                    <button onClick={() => setEditItemForm({...editItemForm, photoUrl: ''})} className="absolute inset-0 bg-black/50 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                                                        <X size={12} />
                                                                    </button>
                                                                </div>
                                                            ) : (
                                                                <label className={`bg-white border border-brand-border rounded h-8 px-2 flex items-center justify-center shrink-0 cursor-pointer hover:bg-brand-primary/5 transition-all ${isUploadingEditPhoto ? 'opacity-50 pointer-events-none' : ''}`}>
                                                                    {isUploadingEditPhoto ? <Loader2 size={12} className="text-brand-secondary animate-spin" /> : <Upload size={12} className="text-brand-secondary" />}
                                                                    <input type="file" accept="image/*" className="hidden" onChange={handleEditPhotoUpload} />
                                                                </label>
                                                            )}
                                                            <input type="text" value={editItemForm.photoUrl} onChange={e => setEditItemForm({...editItemForm, photoUrl: e.target.value})} className="flex-1 text-[10px] px-2 h-8 bg-white border border-brand-border rounded outline-none focus:border-brand-primary" placeholder="Photo URL" />
                                                        </div>
                                                    ...
                                                    <div className="flex items-start justify-between cursor-pointer group/item" onClick={() => { setEditingItemId(item.id); setEditItemForm({ name: item.name, sku: item.sku || '', quantity: item.quantity, photoUrl: item.photoUrl || '' }); }} title="Click to edit item">
                                                        <div>
                                                        ...
                                                        <div className="text-right flex items-center gap-2 pr-4 cursor-auto" onClick={e => e.stopPropagation()}>
                                                            <button onClick={() => setPrintingBox({ pallet: activePallet, box: activeBox, type: 'single_item', item: item })} className="opacity-0 group-hover:opacity-100 bg-white border border-brand-border shadow-sm text-brand-primary px-3 py-1.5 rounded-lg font-bold text-[9px] uppercase tracking-widest transition-all hover:bg-black hover:text-white hover:border-black flex gap-1.5 items-center mr-2">
             ...
             <div className="bg-brand-bg rounded-2xl shadow-2xl max-w-5xl w-full h-full max-h-[90vh] flex flex-col overflow-hidden relative">
                ...
                <div className="flex-1 p-8 overflow-y-auto flex items-start justify-center bg-gray-200 print:p-0 print:bg-white custom-scrollbar print-viewport">
                    {(printingBox.type === 'box' && printingBox.box) || (printingBox.type === 'all_boxes_thermal') ? (
                        ...
                        <div className={`flex ${printingBox.type === 'all_boxes_thermal' ? 'flex-col gap-8 print:gap-0 w-full items-center' : 'w-full h-full items-center justify-center'}`}>
                         ...
                         <div className="flex w-full h-full items-center justify-center">
                             ...
                             <div className="flex justify-between items-start mb-4 border-b-4 border-black pb-3 shrink-0">
                                 ...
                                 <div className="text-right">
                                     <div className="text-3xl font-black font-sans uppercase tracking-widest bg-black text-white px-3 py-1.5 inline-block">{printingBox.pallet.type ? printingBox.pallet.type.toUpperCase() : "MASTER"}</div>
                             ...
                             <div className="flex gap-4 flex-1 items-center justify-between min-h-0 pl-2">
                                 <div className="flex-1 shrink-0">
                                     <div className="text-6xl font-black font-sans tracking-tighter leading-none">
                                     ...
                                     <div className="text-xl font-black font-sans uppercase tracking-widest mt-1 border-t-2 border-black pt-1 max-w-[150px]">
                                 ...
                                 <div className="shrink-0 flex flex-col items-center justify-center border-l-4 border-black pl-6 pr-2">
                                     ...
                                     <p className="text-[10px] font-black uppercase tracking-widest text-center w-full text-black leading-tight whitespace-pre-wrap">
                     ...
                     ) : printingBox.type === 'single_item' && printingBox.item && printingBox.box ? (
                         /* Thermal Label for a Single Item */
                         <div className="flex w-full h-full items-center justify-center">
                             <div className="print-page-wrapper">
                                 <div className="bg-white p-6 border-4 border-black print-label-container my-auto print:shadow-none print:border-none print:m-0 overflow-hidden flex flex-col" style={{ width: '4in', height: '2in', boxSizing: 'border-box' }}>
                                     <div className="flex justify-between items-start mb-2 border-b-[3px] border-black pb-2 shrink-0">
                                         <div>
                                             <h1 className="font-sans text-2xl font-black uppercase tracking-tighter leading-none line-clamp-1">{printingBox.item.name}</h1>
                                             <p className="text-xs font-bold uppercase tracking-widest mt-1">SKU: {printingBox.item.sku || 'N/A'}</p>
                                         </div>
                                     </div>
        
                                     <div className="flex gap-4 flex-1 items-center justify-between min-h-0 pl-2">
                                         <div className="flex-1 shrink-0">
                                             <div className="text-xs font-black font-sans uppercase tracking-widest mb-1 opacity-70">Located In:</div>
                                             <div className="text-xl font-bold font-sans uppercase leading-none max-w-[140px] truncate">{printingBox.pallet.name}</div>
                                             <div className="text-xs font-bold font-sans uppercase mt-1">{printingBox.box.name}</div>
                                         </div>
                                         
                                         <div className="shrink-0 flex flex-col items-center justify-center pl-4 pr-2">
                                             <div className="p-1 border-2 border-black bg-white">
                                                 <QRCode value={`${window.location.hostname === 'localhost' ? 'https://print-shop-os.vercel.app' : window.location.origin}/inventory/scan?p=${printingBox.pallet.id}&b=${printingBox.box.id}&i=${printingBox.item.id}`} size={64} level="L" />
                                             </div>
                                         </div>
                                     </div>
                                 </div>
