...
import { Package, Box as BoxIcon, ArrowRight, CheckCircle2, QrCode, Copy, LogIn, LogOut } from 'lucide-react';
              ...
              <div className="w-full max-w-md space-y-4 mb-6 relative">
                  ...
                  <div className="bg-white p-6 rounded-2xl shadow-lg border border-brand-border">
                      <div className="flex justify-between items-center pb-4 border-b mb-4">
                          <span className="text-xs uppercase font-bold tracking-widest text-brand-secondary">Payload Status</span>
                          <span className={`px-2 py-1 text-[10px] uppercase font-bold tracking-widest rounded ${currentPallet.status === 'Checked Out' ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
                              {currentPallet.status === 'Checked Out' ? 'OUT' : 'IN WAREHOUSE'}
                          </span>
                      </div>
                      
                      {currentPallet.status === 'Checked Out' ? (
                          <button 
                              onClick={async () => {
                                  if (window.confirm("Check this payload back into the warehouse?")) {
                                      try {
                                          await setDoc(doc(db, currentPallet._collection || 'pallets', currentPallet.id), { 
                                              status: 'In Warehouse',
                                              checkedOutTo: null,
                                              checkoutDate: null
                                          }, { merge: true });
                              ...
                              className="w-full bg-emerald-50 text-emerald-600 py-4 rounded-xl font-bold uppercase tracking-widest text-xs flex items-center justify-center gap-2 border border-emerald-200 active:bg-emerald-600 active:text-white transition-colors"
                          >
                              <LogIn size={16} /> Check In Payload
                          </button>
                      ) : (
                          <button 
                              onClick={async () => {
                                  const who = window.prompt("Where is this payload going?");
                                  if (who) {
                                      try {
                                          await setDoc(doc(db, currentPallet._collection || 'pallets', currentPallet.id), { 
                                              status: 'Checked Out',
                              ...
                              className="w-full bg-amber-50 text-amber-600 py-4 rounded-xl font-bold uppercase tracking-widest text-xs flex items-center justify-center gap-2 border border-amber-200 active:bg-amber-600 active:text-white transition-colors"
                          >
                              <LogOut size={16} /> Check Out Payload
                          </button>
                      )}
                  </div>
